...
class Curl {
	...
	public function post($params = array(), $options = array())
	{
		...
		if (is_array($params))
		{
			$params['endeavor-auth'] = SSI_CURL_VAR;
	...
	public function option($code, $value)
	{
		...
		{
			if(!strstr($code,'CURLOPT_'))//if it doeds not contain this, then append  it
	...
	public function create($url)
	{
		...
		//set required informatino
		$this->options(array(
			CURLOPT_USERAGENT => SPECTRUM_USER_AGENT,
	...
	/**
	* @author Sam
	* independent of other functionsi nthe library, it just sends a simple post
	* used for beanstream batch file, which wasnt workign with the other optoins
	* 
	* @param mixed $url
	* @param mixed $post
	*/
	public function simple_post($post_url,$params)
	{
		$this->session  = curl_init();
		curl_setopt($this->session , CURLOPT_HEADER, 0);
		curl_setopt($this->session , CURLOPT_VERBOSE, 0);
		curl_setopt($this->session , CURLOPT_RETURNTRANSFER, true);
		curl_setopt($this->session , CURLOPT_USERAGENT, "Mozilla/4.0 (compatible;)");
		curl_setopt($this->session , CURLOPT_URL, $post_url);
	...
	/**
	* added this as a simple wrapper to the curl_error method for the latest session
	* used with any method that sets the 
